...
For `to-tickets`, first read and follow [Ticket Review Publication](references/ticket-review-publication.md). Read the configured tracker extension before invoking the target, allocate or resume its stable publication-run identity, and convey the exact durable draft surface, representation, `review-pending` state, identity fields, and no-direct-ready rule. If the configured Local Markdown surface is unwritable or its adapter is unsafe, report the set as unpublished; do not substitute a second draft representation or call conversation-only review a successful mutation.
