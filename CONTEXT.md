...
**Attempt**:
An in-progress try at a ticket. It may be represented by claim metadata, `solve-in-progress`, a branch, a worktree, a ticket comment/note, or current session context. A meaningful stopped Attempt creates an outcome-aware recovery Solve Record; a fully cleaned transient Attempt remains recordless.
_Avoid_: Solve record, delivery receipt, run log

**Agent Brief**:
An optional, non-duplicative execution delta on a ticket, usually produced while shaping tickets for AFK work. It may contain only Constraints, Validation, and optional Hints; solve treats it as subordinate approved input rather than a required schema field.
...
**Execution Digest**:
A conditional, external ticket-level working file at `.scratch/<feature>/execution-digests/<digest-key>.md`. It preserves execution strategy, touched surfaces, key risks, validation plan, and record-worthy decisions or deviations without storing raw exploration notes; durable items are distilled into the outcome Solve Record.
_Avoid_: Plan approval, full transcript, subagent log, PRD replacement

**Post-Execution Review**:
A multi-perspective agent self-check of the integrated candidate after final validation and before solve-record finalization. It checks the final candidate against tickets, acceptance criteria, execution digests, record-worthy decisions, side effects, regressions, and validation gaps; fixable findings are fixed before handoff.
...
**Solve Record**:
A short, maintainer-facing outcome receipt at a meaningful Attempt handoff. A candidate receipt summarizes a finished, checkable merge candidate; a recovery receipt preserves the findings, blocker, and retained resources needed to resume a meaningful stopped Attempt.
