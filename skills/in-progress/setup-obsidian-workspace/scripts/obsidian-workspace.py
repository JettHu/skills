...
def load_query():
    """Resolve one explicit dependency set, never search the target project or HOME."""
    explicit = os.environ.get('JETT_ULTRA_SKILL_DIR')
    skill = Path(__file__).resolve().parents[1]
    if explicit is not None:
        ultra = Path(explicit).expanduser()
        if not explicit or not ultra.is_absolute():
            raise RuntimeError('JETT_ULTRA_SKILL_DIR must be an absolute Ultra skill directory')
    elif skill.parent.name == 'in-progress' and skill.parent.parent.name == 'skills':
        ultra = skill.parent.parent / 'engineering/ultra'
    else:
        ultra = skill.parent / 'ultra'
    scripts = ultra.resolve() / 'scripts'
    source = scripts / 'tracker_snapshot.py'
    records = scripts.parent.parent / 'solve-records/scripts/solve-records.py'
    if not source.is_file() or not records.is_file():
        raise RuntimeError(f'Install compatible ultra and solve-records together; expected {source} and {records}. '
                           'For a separate dependency installation set JETT_ULTRA_SKILL_DIR explicitly.')
    loaded = sys.modules.get('tracker_snapshot')
    if loaded is not None and Path(loaded.__file__).resolve() != source:
        raise RuntimeError('A different Tracker Snapshot is already loaded; run the renderer in a fresh process')
    ...
    try:
        ...
        helper = module.records_helper()
        if not all(callable(getattr(helper, name, None))
                   for name in ('discover', 'record_summary', 'cleanup_plan')):
            raise RuntimeError('Unsupported solve-records helper; install compatible solve-records')
        return module
    except (ImportError, AttributeError, SyntaxError) as error:
        raise RuntimeError(f'Incompatible Ultra/Solve Records dependency at {scripts}: {error}') from error
    finally:
        sys.path.pop(0)
...
def refresh(config):
    ...
    if not (root / 'Home.md').exists() and any(root.iterdir()):
        ...
        if any(path.name not in {'.refresh.lock', 'generations', '.setup.lock', '.workspace-config.json'} for path in root.iterdir()):
