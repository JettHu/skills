...
Read the shared index `docs/agents/ultra-tracker.md` and its selected adapter capability document before discovery. Use only the configured adapter and fail closed when the mutation contract is missing, malformed, or unsupported. Provisional, staged, partially promoted, superseded, or membership-unverified Tickets are outside solve discovery. A remote tracker without a mutation adapter is read-only.
...
For each claimed Ticket, run the Pre-Implementation Checkpoint after Claim and before implementation edits.

### Bootstrap: Design Context and Risk Routing

Build a compact **Design Context** index to identify the scope, constraints, and unresolved questions before deciding the execution plan. Route substantial code exploration from this minimum context; the root does not need to reconstruct each implementation path before delegation. Read, in order: the selected Ticket (body, acceptance criteria, comments, and optional Agent Brief); the live Claim/discovery snapshot; the authoritative Ticket pointers; then only the current worktree/branch facts and source material needed for the changed surface. The runtime-injected `AGENTS.md` is already active guidance, not routine rereading work. Do not scan all ADRs, all context files, or unrelated `.scratch` content. Treat an explicitly superseded document as historical context, not live requirements.

`## Context` is an optional navigation contract, not a parser gate or a second source of requirements. For every pointed source, read the exact selected document at its declared source branch/ref and retain its classification and relevance in the index. For repository documents, verify the document/ref pair rather than reading the same path from the executor's current `HEAD`; a non-repository source identifies its immutable version or external revision. Use `context_pointer_ref_unavailable_authoritative` only when the declared ref itself does not resolve. If that ref resolves but the authoritative document is absent, stop before implementation with `context_pointer_missing_authoritative`; if it resolves to multiple candidates or conflicting locations, stop with `context_pointer_ambiguous_authoritative`. Route these diagnostics through the existing `needs-info` or `ready-for-human` boundary as the authoritative context requires. If a nonessential pointer is unavailable, report `context_pointer_unavailable_nonessential` and continue with the selected sources. Never silently substitute a different document or ref.

When the changed surface clearly identifies one omitted decision document, make at most one targeted fallback lookup and record its result. A discovered conflict blocks implementation; an absent nonessential document does not. This fallback does not scan all ADRs, all context files, or unrelated `.scratch` content, and it never changes the Ticket's selected source without the owning shaping or human decision.

Record the authoritative sources, consulted sources, diagnostics, and any unresolved conflict in the active context; put the index in the Execution Digest only when the Attempt is otherwise digest-worthy. A simple Ticket needs only a one-line index such as `Design Context: ADR-0012 (authoritative); CONTEXT.md (nonessential); no unresolved conflict`.
...
- `L2`: any state, permission, data, concurrency, migration, recovery, and cross-repository coupling; or any cross-module or cross-group coupling, public contract, production/configuration effect, external dependency, or non-obvious validation; use the applicable exploration, Digest, Pre-Edit Plan Review, one bounded independent evidence pass, integration validation, and Post-Execution Review gates.
...
Map each material predicate to its extra gates: cross-module or cross-group coupling enables targeted exploration, Group Review, integration validation, and Post-Execution Review; state, permission, data, concurrency, or migration work enables Design Context, one bounded independent evidence pass, integration/final validation, and an explicit safety/evidence disposition; recovery or cross-repository coupling enables the Digest, a narrow boundary check, and recovery evidence; production or configuration effects enable rollout/config review and release-boundary evidence; external dependencies enable conditional source-backed research and corresponding validation; non-obvious validation enables the Digest and Pre-Edit Plan Review; L3 governance retains the full evidence and human-boundary policy named by the Ticket or repository.

Each evidence objective has one owner. When target-native evidence already covers its objective, target-native evidence is the sole owner and no additive pass repeats it. For a high-risk Ticket, select at most one distinct bounded independent evidence pass for the uncovered objective; it may be an isolated acceptance reviewer, an explicitly requested code review, or one selected Group or Post-Execution Review, but never a duplicate of native evidence. The root still performs its required self-checks, Final Validate, and requirement-to-evidence audit; those are not extra independent passes.
...
When an assigned implementation subagent needs this exploration, let it retain the context and continue through implementation after the root incorporates its findings into any required Checkpoint and Pre-Edit Plan Review. Keep it read-only until those gates and the assigned worktree are verified. A separate exploration agent is useful only when it supplies an independent question or evidence that the implementation owner does not already have to gather. Stage Ownership remains the implementation authority.

The root consumes the returned evidence, checks critical cross-surface contracts, and resolves conflicts. Reopen source only for a material evidence gap, conflicting finding, or changed code/context; use the supplied locations rather than repeating broad searches. Small local questions, evidence already in context, or unavailable/uneconomical delegation can be handled directly without creating a parallel pass. Ground capability fallback in the active tool surface or an actual unavailable result, not an assumed tool name.

Exploration is complete when the affected entry points, critical contracts and call paths, and validation approach are identified, with no unresolved uncertainty blocking implementation. Record remaining nonblocking uncertainty beside its follow-up check and proceed through the enabled gates. Further exploration is driven by a specific new question. After compaction or handoff, resume from these conclusions and source pointers, supplementing only missing or stale evidence.
...
- Complex or cross-cutting group: use the checkpoint's one selected independent pass only when this group owns its uncovered evidence objective.
