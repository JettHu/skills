...
For `to-tickets`, first read and follow [Ticket Review Publication](references/ticket-review-publication.md). Read the configured tracker extension before invoking the target, allocate or resume its stable publication-run identity, and convey the exact durable draft surface, representation, `review-pending` state, identity fields, and no-direct-ready rule. For GitHub or GitLab, use only the configured `remote-review-pending` or `local-staging` policy, including its provider marker, relationship operation, recovery surface, and Claim gate. If the configured durable Local Markdown or local-staging surface is unwritable, report the degradation; do not silently substitute the other strategy or call conversation-only review a successful mutation.
...
For canonical shaping targets `to-spec` and `to-tickets`, review the exact generated artifact on its configured draft or publication surface. For `to-tickets`, rediscover the complete formal set by publication-run identity, register exact membership, and keep every member non-claimable throughout review. The fresh-context reviewer checks Spec coverage, canonical terminology, independent acceptance, context-window sizing, validation, source pointers, and true blocker edges. The main Agent fixes every finding derivable from approved context and current code in those same artifacts: factual drift, canonical Spec/Ticket terminology (use `Ticket`, not `Issue`), acceptance or validation gaps, sizing and split/merge repairs, blocker-edge corrections, and formatting or backlink defects. Re-run the affected review after each repair; complete only when no fixable finding or unresolved human-owned choice remains. Ask the user only for an unresolved scope, product-semantic, ownership, release-policy, data/security-policy, architecture, significant UX, or missing-core-requirement choice.

For `to-tickets`, the main Agent then runs the configured complete-set promotion transaction from [Ticket Review Publication](references/ticket-review-publication.md). Promotion requires pre- and post-mutation re-reads of membership, bodies, source pointers, blocker targets, review state, and concurrent-change evidence. GitHub and GitLab remote review-pending runs also re-read the configured provisional marker and provider-native or textual-fallback relationships; local-staging runs retain their manifest until remote IDs, edges, and final ready state verify. Interrupted or cancelled runs remain durably resumable and non-claimable; only the fully verified set becomes `ready-for-agent`.
